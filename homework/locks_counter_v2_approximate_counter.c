...
int record_stats(const char* type, int num_threads, int num_loops, long micro_seconds, int update_frequency) {
    ...
    if (current_position == 0) {  // if the position is 0, the file is empty; write headers
        fprintf(file, "Type,# Threads,# Loops,Microseconds,Update Frequency\n");
    }
    fseek(file, 0, SEEK_END);  // move the the pointer to the end of the file to append data
    fprintf(file, "%s, %d, %d, %ld, %d\n", type, num_threads, num_loops, micro_seconds, update_frequency);
    fclose(file);
}
...
int main(int argc, char *argv[]) {
    ...
    printf("----\n\n");
    int status = record_stats("Locking V2 - Approx", num_threads, num_loops, micro_seconds, update_frequency);
